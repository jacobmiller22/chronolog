...
class GoogleLogApi(api.LogApi):
    ...
    def __set_path_parent_ids(self):
        """
        ...
        try:
            ...
            raise ValueError("Invalid google credentials") from credentials_error
    ...
    def _auth(self) -> bool:
        ...
        """
        print("test")
        client_secrets_path = os.path.normpath(os.path.join(ROOT_DIR, "credentials", GOOGLE_CLIENT_SECRETS_FILE))
        print(client_secrets_path)
        flow = InstalledAppFlow.from_client_secrets_file(client_secrets_path, scopes=GOOGLE_DRIVE_API_SCOPES)
    ...
    def auth(self) -> bool:
        ...
        creds = None
        token_path = os.path.abspath(os.path.join(os.path.dirname(self._config.get_path()), "google_token.json"))
        ...
        if not creds or not creds.valid:
            ...
            try:
                ...
                print("Could not refresh credentials. Reauthenticating...")
                self._auth()
            # else:
    ...
    def _build_batch_requests(self, title: str, date: str, contents: str, doc: dict):
        ...
        if date_exists:
            insertions = [
                {
                    "insertText": {
                        ...
                        "location": {
                            "index": len(date)
                            + len(doc["body"]["content"][2]["paragraph"]["elements"][0]["textRun"]["content"])
                            + 1
                        },
                    }
                },
            ...
            insertions = [
                {"insertText": {"text": date + "\n", "location": {"index": 1}}},
                {"insertText": {"text": contents + "\n", "location": {"index": len(date) + 2}}},
            ]
            ...
            *insertions,
            {
                "updateParagraphStyle": {
                    "range": {"startIndex": 1, "endIndex": len(date) + 1},
                    "paragraphStyle": {
            ...
            {
                "updateParagraphStyle": {
                    "range": {"startIndex": len(date) + 2, "endIndex": len(date) + len(contents) + 2},
                    "paragraphStyle": {
    ...
    def _find_target_document(self, ds: Resource, q: str) -> Union[str, None]:
        ...
        while True:
            ...
            if len(items) == 1:
                return items[0]["id"]
            if len(items) > 1:
                raise ValueError("Multiple documents with the same title exist in the path")
    ...
    def __create_file(
        ...
        """
        # Create the document
        file_metadata = {"name": title, "mimeType": mime_type, "parents": parents}
    ...
    def upload_log(self, date: datetime, log_contents: str) -> bool:
        ...
        with build("drive", "v3", credentials=self._credentials) as ds:
            target_log_id = self._find_target_document(
                ds=ds,
                q=f"name='{group}' and mimeType='application/vnd.google-apps.document' and '{self._config.get('google_drive._parents_path')[-1]}' in parents",
            )
            ...
            if target_log_id is None:
                ...
                target_log_id = self.__create_file(
                    ds=ds,
                    title=group,
                    parents=[self._config.get("google_drive._parents_path")[-1]],
        ...
        with build("docs", "v1", credentials=self._credentials) as docs:
            ...
            docs.documents().batchUpdate(
                documentId=target_log_id,
                body={
                    "requests": self._build_batch_requests(
                        title=group, date=date.strftime("%m/%d/%Y"), contents=log_contents, doc=log
                    )
                },
            ).execute()
